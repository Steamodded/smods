...
function G.UIDEF.deck_preview(args)
	...
	for _, rank in ipairs(rank_name_mapping) do
		...
		if count == 0 and SMODS.Ranks[rank].in_pool and not SMODS.is_in_pool(SMODS.Ranks[rank], {suit=''}) then
			hidden_ranks[rank] = true
		end
	end
	local hidden_suits = {}
	for _, suit in ipairs(suit_map) do
		if suit_counts[suit] == 0 and SMODS.Suits[suit].in_pool and not SMODS.is_in_pool(SMODS.Suits[suit],{rank=''}) then
...
function G.UIDEF.view_deck(unplayed_only)
	...
	for i = #rank_name_mapping, 1, -1 do
		if rank_tallies[rank_name_mapping[i]] ~= 0 or SMODS.is_in_pool(SMODS.Ranks[rank_name_mapping[i]], {suit=''}) then
	...
	local hidden_suits = {}
	for _, suit in ipairs(suit_map) do
		if suit_tallies[suit] == 0 and SMODS.Suits[suit].in_pool and not SMODS.is_in_pool(SMODS.Suits[suit], {rank=''}) then
			hidden_suits[suit] = true
		end
	end
	local i = 1
...
function get_pack(_key, _type)
    ...
    for k, v in ipairs(G.P_CENTER_POOLS['Booster']) do
		...
		if v.in_pool and type(v.in_pool) == 'function' then
			local res, pool_opts = SMODS.is_in_pool(v)
