...
--#endregion
--#region Flush, Straights, X same, quantum Full House and Two Pair
...
function get_straight(hand, min_length, skip, wrap)
	...
	for _, pcard in ipairs(hand) do
		if SMODS.has_any_rank(pcard, {eval_getting_ranks = {type = "straight"}}) then
			...
			local pcard_ranks = SMODS.get_straight_ranks(pcard:get_ranks({as_objs = true}), true)
...
function get_quantum_pairing(parts, base)
	if not SMODS.optional_features.quantum_fields.rank then return {} end
...
G.FUNCS.change_collab = function(args)
	...
	for k, v in pairs(G.I.CARD) do
		if v.config and v.config.card and v.children.front and not v.front_hidden then
...
function Card:is_face(from_boss)
	...
	if not SMODS.optional_features.quantum_fields.rank then
		...
		if SMODS.has_any_rank(self) then
			return true
		end

		for rank, _ in pairs(self:get_ranks({as_objs = true})) do
...
function Card:set_ability(center, initial, delay_sprites)
	...
	SMODS.clear_quantum_cache(self)
