...
function G.UIDEF.view_deck(unplayed_only)
	...
	for k, v in ipairs(G.playing_cards) do
		if v.ability.name ~= 'Stone Card' and (not unplayed_only or ((v.area and v.area == G.deck) or v.ability.wheel_flipped)) then
			if v.ability.wheel_flipped and not (v.area and v.area == G.deck) and unplayed_only then
				wheel_flipped =
					wheel_flipped + 1
			end
			local v_nr, v_ns = SMODS.has_no_rank(v), SMODS.has_no_suit(v)
			...
			if v.base.value and not v_nr and not v.debuff then
				mod_rank_tallies[v.base.value] = mod_rank_tallies
					[v.base.value] + 1
			end
		end
	end
...
G.FUNCS.your_suits_page = function(args)
	...
	for k, v in ipairs(G.playing_cards) do
		if v.ability.name ~= 'Stone Card' and (not view_deck_unplayed_only or ((v.area and v.area == G.deck) or v.ability.wheel_flipped)) then
			if v.ability.wheel_flipped and not (v.area and v.area == G.deck) and view_deck_unplayed_only then
				wheel_flipped =
					wheel_flipped + 1
			end
			--For the suits
...
G.FUNCS.change_collab = function(args)
	...
	if swap_node then
		...
		local new_palette_selector = {
			...
			nodes = {
				create_option_cycle({
					options = palette_loc_options,
					w = 5.5,
					cycle_shoulders = false,
					curr_suit = args
						.cycle_config.curr_suit,
					curr_skin = deckskin_key,
					opt_callback = 'change_colour_palette',
					current_option =
						selected_palette,
					colour = G.C.ORANGE,
					focus_args = { snap_to = true, nav = 'wide' }
...
function Blind:debuff_hand(cards, hand, handname, check)
	...
	for i = 1, #cards do
		...
		SMODS.calculate_context(
			{ modify_scoring_hand = true, other_card = cards[i], full_hand = cards, scoring_hand = scoring_hand },
			effects)
				...
				SMODS.merge_defaults(contexts,
					{
						...
						other_card = self,
						new_rank = new_rank,
						old_rank = self.base.id,
						rank_increase = ((self.base.id < new_rank) and true) or
							false
					})
			end
		end
