...
function UIElement:draw_self()
	local function _draw()
		...
		if self.config.colour[4] > 0.01 then
			if self.UIT == G.UIT.T and self.config.scale then
				local function _draw_text(shadow)
					if shadow then
						love.graphics.draw(
				...
				local function _draw_text_layer(shader, send, shadow)
					local args = {
						G.TIMERS.REAL/28,
						G.TIMERS.REAL
					}
					...
					if shader == "none" or shader == "dissolve" then
						_draw_text(not not shadow)
						return
					elseif send then
						for _, v in ipairs(send) do
							local val = v.val or (v.func and v.func(self)) or v.ref_table[v.ref_value]
							-- TARGET: Text shader: Convert v.val to a number if your mod adds an alternate number data type (ala Talisman)
					...
					elseif shader == "vortex" then
						...
						local text_dims = {self.config.text_drawable:getDimensions()}

						G.SHADERS[shader]:send(key, args)
						G.SHADERS[shader]:send("text_details", {self.VT.x * tile_scale, self.VT.y * tile_scale, self.VT.w * tile_scale, self.VT.h * tile_scale})
						G.SHADERS[shader]:send("text_scale", self.VT.scale)
						G.SHADERS[shader]:send("text_rot", self.VT.r)
					...
					if (self.config.shadow or (self.config.button_UIE and button_active)) and G.SETTINGS.GRAPHICS.shadows == 'On' then
						...
						if self.config.shader then
							...
							if type(self.config.shader) == "string" then
								_draw_text_layer(self.config.shader, nil, true)
							...
							elseif type(self.config.shader) == "table" then
								...
								if self.config.shader.shader then
									_draw_text_layer(self.config.shader.shader, self.config.shader.send, true)

								-- list of shaders
								elseif #shader > 0 then
									for _, v in ipairs(self.config.shader) do
										if type(v) == "string" then
											_draw_text_layer(v.shader, nil, true)
										elseif type(v) == "table" then
											_draw_text_layer(v.shader, v.send, true)
										end
									end
								end
							end
				...
				if self.config.shader then
					-- simple single shader
					if type(self.config.shader) == "string" then
						_draw_text_layer(self.config.shader, nil)

					-- more complex shader calls
					elseif type(self.config.shader) == "table" then
						-- one shader pass with a custom send
						if self.config.shader.shader then
							_draw_text_layer(self.config.shader.shader, self.config.shader.send)

						-- list of shaders
						elseif #shader > 0 then
							for _, v in ipairs(self.config.shader) do
								if type(v) == "string" then
									_draw_text_layer(v.shader, nil)
								elseif type(v) == "table" then
									_draw_text_layer(v.shader, v.send)
								end
							end
						end
					end
    ...
    if self.config.shader and self.states.visible and self.UIT ~= G.UIT.T then
        -- simple single shader
        if type(self.config.shader) == "string" then
...
function DynaText:get_letter_shaders(letter, shadow, shader, send)
    local shaders = {}
...
function DynaText:set_letter_shader(letter, shadow, shader, send)
    if not letter or not shader then
        if self.shadered then love.graphics.setShader() end
        self.shadered = nil
        return
    end
