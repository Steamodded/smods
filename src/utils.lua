    ...
    if (key == 'mult' or key == 'h_mult' or key == 'mult_mod') and amount then
        ...
        if not effect.remove_default_message then
            if from_edition then
                card_eval_status_text(scored_card, 'jokers', nil, percent, nil,
                    {
                        message = localize { type = 'variable', key = amount > 0 and 'a_mult' or 'a_mult_minus', vars = { amount } },
                        mult_mod =
                            amount,
    ...
    if (key == 'p_dollars' or key == 'dollars' or key == 'h_dollars') and amount then
        ...
        if not effect.remove_default_message then
            if effect.dollar_message then
                ...
                card_eval_status_text(
                    effect.message_card or effect.juice_card or scored_card or effect.card or effect.focus, 'dollars',
                    amount,
                    percent)
            end
        end
    ...
    if (key == 'x_chips' or key == 'xchips' or key == 'Xchip_mod') and amount ~= 1 then
        ...
        if not effect.remove_default_message then
            if from_edition then
                card_eval_status_text(scored_card, 'jokers', nil, percent, nil,
                    {
                        message = localize { type = 'variable', key = amount > 0 and 'a_xchips' or 'a_xchips_minus', vars = { amount } },
                        Xchips_mod =
                            amount,
                        colour = G.C.EDITION,
                        edition = true
    ...
    if (key == 'x_mult' or key == 'xmult' or key == 'Xmult' or key == 'x_mult_mod' or key == 'Xmult_mod') and amount ~= 1 then
        ...
        if not effect.remove_default_message then
            if from_edition then
                card_eval_status_text(scored_card, 'jokers', nil, percent, nil,
                    {
                        message = localize { type = 'variable', key = amount > 0 and 'a_xmult' or 'a_xmult_minus', vars = { amount } },
                        Xmult_mod =
                            amount,
                        colour = G.C.EDITION,
                        edition = true
...
function SMODS.update_hand_limit_text(play, discard)
    if play then
        SMODS.hand_limit_strings.play = G.GAME.starting_params.play_limit ~= 5 and
            localize('b_limit') .. math.max(1, G.GAME.starting_params.play_limit) or ''
    end
    if discard then
        SMODS.hand_limit_strings.discard = G.GAME.starting_params.discard_limit ~= 5 and
            localize('b_limit') .. math.max(0, G.GAME.starting_params.discard_limit) or ''
    end
end
    ...
    local fixed = SMODS.calculate_context(
        {
            ...
            identifier = identifier,
            numerator =
                additive.numerator or base_numerator,
            denominator = additive.denominator or base_denominator
        }, nil, not from_roll)
